...
func ValidKubeNodeListRequest(data interface{}, _ *gin.Context) map[string][]string {
	rules := govalidator.MapData{
		...
		"limit": []string{"required", "numeric", "min:1", "max:200"},
	}
...
func ValidKubeNodePodsRequest(data interface{}, _ *gin.Context) map[string][]string {
	...
	msgs := govalidator.MapData{
		...
		"limit": {"required:每页数量必填", "numeric:每页数量必须为数字", "min:每页数量不能小于 1", "max:每页数量不能超过 200"},
	}
	return valid.ValidateOptions(data, rules, msgs)
}

func NewKubeNodeEventsRequest() *KubeNodeEventsRequest { return &KubeNodeEventsRequest{} }
...
type KubeNodeMetricsRequest struct {
	Name  string `form:"nodeName" json:"nodeName" valid:"nodeName"` // 节点名，必填
	Page  int    `form:"page"     json:"page" valid:"page"`         // 与 Pod 接口保持一致（即便这里返回单条，也保留字段）
...
func ValidKubeNodeMetricsRequest(data interface{}, ctx *gin.Context) map[string][]string {
	...
	msgs := govalidator.MapData{
		"nodeName": []string{"required: nodeName 不能为空"},
