...
func PatchDaemonSet(ctx context.Context, namespace, name string, patchBytes []byte) (*appv1.DaemonSet, error) {
	// 调用 K8s API 执行 patch
	patched, err := global.KubeClient.AppsV1().
...
func PatchUpdateDaemonSetImage(ctx context.Context, namespace, name, containerName, image string) (*appv1.DaemonSet, error) {
	patchImage, err := patchbuilder.BuildImagePatch(containerName, image)
	if err != nil {
		return nil, err
	}
	return PatchDaemonSet(ctx, namespace, name, patchImage)
}
