...
func CreateServiceFromStatefulSet(
	client *k8s.Client, ctx context.Context,
	...
	if err != nil {
		...
		client.Logger.Errorf("create headless service %s/%s failed: %v", req.Namespace, svc.Name, err)
		return nil, fmt.Errorf("failed to create service %s/%s: %w", req.Namespace, svc.Name, err)
	}

	client.Logger.Infof("headless service %s/%s created successfully", createdSvc.Namespace, createdSvc.Name)
