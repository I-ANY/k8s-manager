...
func CreateSecret(client *k8s.Client, ctx context.Context, req *requests.KubeSecretCreateRequest) (*corev1.Secret, error) {
	...
	if err != nil {
		...
		client.Logger.Errorf("create secret failed: %v", err)
		return nil, err
	}

	client.Logger.Infof("secret %q created successfully", created.Name)
	return created, nil
}
